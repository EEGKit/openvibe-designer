	...
	: m_mapDatabase(mapDatabase), m_currentInterpolation(interpolation)
{
	...
	//color palettes
	palette[0].red    = 255 * 65535 / 255;
	palette[0].green  = 0 * 65535 / 255;
	palette[0].blue   = 0 * 65535 / 255;
	palette[1].red    = 234 * 65535 / 255;
	palette[1].green  = 1 * 65535 / 255;
	palette[1].blue   = 0 * 65535 / 255;
	palette[2].red    = 205 * 65535 / 255;
	palette[2].green  = 0 * 65535 / 255;
	palette[2].blue   = 101 * 65535 / 255;
	palette[3].red    = 153 * 65535 / 255;
	palette[3].green  = 0 * 65535 / 255;
	palette[3].blue   = 178 * 65535 / 255;
	palette[4].red    = 115 * 65535 / 255;
	palette[4].green  = 1 * 65535 / 255;
	palette[4].blue   = 177 * 65535 / 255;
	palette[5].red    = 77 * 65535 / 255;
	palette[5].green  = 0 * 65535 / 255;
	palette[5].blue   = 178 * 65535 / 255;
	palette[6].red    = 0 * 65535 / 255;
	palette[6].green  = 0 * 65535 / 255;
	palette[6].blue   = 152 * 65535 / 255;
	palette[7].red    = 0 * 65535 / 255;
	palette[7].green  = 97 * 65535 / 255;
	palette[7].blue   = 121 * 65535 / 255;
	palette[8].red    = 0 * 65535 / 255;
	palette[8].green  = 164 * 65535 / 255;
	palette[8].blue   = 100 * 65535 / 255;
	palette[9].red    = 0 * 65535 / 255;
	palette[9].green  = 225 * 65535 / 255;
	palette[9].blue   = 25 * 65535 / 255;
	palette[10].red   = 150 * 65535 / 255;
	palette[10].green = 255 * 65535 / 255;
	palette[10].blue  = 0 * 65535 / 255;
	palette[11].red   = 200 * 65535 / 255;
	palette[11].green = 255 * 65535 / 255;
	palette[11].blue  = 0 * 65535 / 255;
	palette[12].red   = 255 * 65535 / 255;
	palette[12].green = 255 * 65535 / 255;
	palette[12].blue  = 0 * 65535 / 255;
...
bool CTopographicMap2DView::setSampleValuesMatrix(IMatrix* matrix)
{
	...
	const int colorStartIdx = 0;
	const int colorEndIdx   = NB_COLORS - 1;
...
void CTopographicMap2DView::drawFace(size_t /*X*/, size_t /*Y*/, size_t /*width*/, size_t /*height*/) const
{
	...
	if (m_currentView == EView::Top)
	{
		...
		const gint lx = gint(size_t(skullCenterX + 1.0 * m_skullDiameter / 2 * cos(Deg2Rad(90 + noseHalfAngle))));
		const gint ly = gint(size_t(skullCenterY - 1.0 * m_skullDiameter / 2 * sin(Deg2Rad(90 + noseHalfAngle))));
...
void CTopographicMap2DView::resizeData()
{
	...
	m_paletteWindowHeight = size_t(0.1 * height);
	...
	else if (m_currentView == EView::Back)
	{
		...
		const size_t neckProtrudingHeight = size_t(0.072 * headMaxSize);
		...
		//neck extremities
		m_leftNeckX  = m_skullOutlineLeftPointX + size_t(0.025F * m_skullDiameter);
	...
	else if (m_currentView == EView::Left || m_currentView == EView::Right)
	{
		...
		if (m_currentView == EView::Left)
		{
			...
			m_noseTipY = m_noseBumpY + size_t(0.03F * m_skullDiameter);//size_t(0.021f * m_skullDiameter);
			//nose base
			m_noseBaseX = m_noseTipX + size_t(0.1F * m_skullDiameter);
			m_noseBaseY = m_noseTipY;
			//nose bottom
			m_noseBottomX = m_noseBaseX;
			m_noseBottomY = m_noseBaseY + size_t(0.02F * m_skullDiameter);//size_t(0.016f * m_skullDiameter);
		}
		else
		{
			//X coordinate of nose tip
	...
	m_cellSize                          = size_t(m_skullDiameter * cellOverSkullSizeRatio);
	...
	m_gridSize = size_t(ceil(m_skullDiameter / double(m_cellSize)));
