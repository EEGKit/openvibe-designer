...
namespace Mensia
{
	namespace AdvancedVisualization
	{
		...
		typedef TBoxAlgorithmContinuousVizDesc<TRendererProto<ERendererType::XYZPlot>> XYZPlot;
		...
		typedef TBoxAlgorithmInstantVizDesc<TRendererProto<ERendererType::XYZPlot>> IXYZPlot;
		...
		typedef TBoxAlgorithmInstantVizDesc<TRendererProto<ERendererType::Topography2D>, CRulerBottomTexture> Topography2D;
		typedef TBoxAlgorithmInstantVizDesc<TRendererProto<ERendererType::Topography3D>, CRulerBottomTexture> Topography3D;
		typedef TBoxAlgorithmInstantVizDesc<TRendererProto<ERendererType::Loreta>, CRulerBottomTexture, TBoxAlgorithmInstantLoretaViz> SLoreta;
		typedef TBoxAlgorithmInstantVizDesc<TRendererProto<ERendererType::Cube>, CRulerBottomTexture> Cubes;
	} // namespace AdvancedVisualization
} // namespace Mensia
...
OVP_Declare_Begin()
	...
	OVP_Declare_New(
		Bitmap("Continuous Bitmap", OID(0x0BE99978487D3DC6), OID(0x9E1CD34586569E7E), "0.0.0", "0.0.0",
			CParameterSet(I_Matrix, I_Stimulations, S_ChannelLocalisation, S_TemporalCoherence, S_TimeScale, S_ElementCount, S_DataScale, S_Caption,
				S_ColorGradient, P_None),
			"Displays the input matrices as a map of colored tiles, or bitmap, continuously.", ""))
	OVP_Declare_New(
		Oscilloscope("Continuous Oscilloscope", OID(0x6D63896D86685134), OID(0x0842BCD1D53C1C89), "0.0.0", "0.0.0",
			CParameterSet(I_Signal, I_Stimulations, S_ChannelLocalisation, S_TemporalCoherence, S_TimeScale, S_ElementCount, S_DataPositive, S_DataScale,
				S_Caption, S_Translucency, S_Color, P_None),
			"Displays the input matrices as a series of curves.\nChannels are vertically distributed.", ""))
	OVP_Declare_New(
		Bars("Continuous Bars", OID(0xE6B2C8A8B43DE2C3), OID(0x24510F3EFCBE83A2), "0.0.0", "0.0.0",
			CParameterSet(I_Matrix, I_Stimulations, S_ChannelLocalisation, S_TemporalCoherence, S_TimeScale, S_ElementCount, S_DataPositive, S_DataScale,
				S_Caption, S_Translucency, S_ColorGradient, P_None),
			"Displays the input matrices as a series of colored bars.\nChannels are vertically distributed.", ""))
	OVP_Declare_New(
		MOscilloscope("Continuous Multi Oscilloscope", OID(0xD4C4DCEE6F5D47E1), OID(0xA927FB20C4089F99), "0.0.0", "0.0.0",
			CParameterSet(I_Signal, I_Stimulations, S_ChannelLocalisation, S_TemporalCoherence, S_TimeScale, S_ElementCount, S_DataPositive, S_DataScale,
				S_Caption, S_Translucency, S_ColorGradient, P_None),
			"Displays the input matrices as a series of curves.\nAll channels are rendered on the same vertical axis, with different colors.", ""))
	OVP_Declare_New(
		XYZPlot("Continuous XYZ Plot", OID(0xAAFD16CF57DC7138), OID(0xDDA9A9D9D04F2C14), "0.0.0", "0.0.0",
	...
	OVP_Declare_New(
		IBitmap("Instant Bitmap", OID(0x942CDA76A437E83E), OID(0xF862765845247823), "0.0.0", "0.0.0",
	...
	OVP_Declare_New(
		IOscilloscope("Instant Oscilloscope", OID(0x1207B96D19A805B4), OID(0x997B654D0CDB3102), "0.0.0", "0.0.0",
	...
	OVP_Declare_New(
		IBars("Instant Bars", OID(0xD401BE1F25F01E4E), OID(0xECB4608196DA0D49), "0.0.0", "0.0.0",
	...
	OVP_Declare_New(
		MIOscilloscope("Instant Multi Oscilloscope", OID(0x4F142FE87C7773E3), OID(0x072B75A9C28D588A), "0.0.0", "0.0.0",
			CParameterSet(I_Signal, S_ChannelLocalisation, S_DataPositive, S_DataScale, S_Caption, S_Translucency, S_ColorGradient, P_None),
			"Displays each and every input matrix as a series of curves, instantly.\nAll channels are rendered on the same vertical axis, with different colors."
			, ""))
	OVP_Declare_New(
		IXYZPlot("Instant XYZ Plot", OID(0x1353BED819EC86A5), OID(0xDB19374A41A22DD3), "0.0.0", "0.0.0",
	...
	OVP_Declare_New(
		SVBitmap("Stacked Bitmap (Vertical)", OID(0x93F400CF9F6C5AFD), OID(0x9926A761BA82D233), "0.0.0", "0.0.0",
			CParameterSet(I_Matrix, I_Stimulations, S_ChannelLocalisation, S_TemporalCoherence, S_TimeScale, S_ElementCount, S_DataScale, S_Caption,
				S_ColorGradient, P_None),
			"Displays the input matrices as a map of colored tiles, or bitmap, continuously.\nAll the bitmaps are stacked vertically, starting from the bottom edge of the window"
			, ""))
	OVP_Declare_New(
		SHBitmap("Stacked Bitmap (Horizontal)", OID(0x6B22DC653AB0EC0F), OID(0x7B0DDB65FDC51488), "0.0.0", "0.0.0",
			CParameterSet(I_Matrix, I_Stimulations, S_ChannelLocalisation, S_TemporalCoherence, S_TimeScale, S_ElementCount, S_DataScale, S_Caption,
				S_ColorGradient, P_None)
			, "Displays the input matrices as a map of colored tiles, or bitmap, continuously.\nAll the bitmaps are stacked horizontally, starting from the left edge of the window"
			, ""))
	OVP_Declare_New(
		SIBitmap("Instant Bitmap (3D Stream)", OID(0x0C61E7632A5C3178), OID(0xC3CC8B43EE985C1D), "0.1.0", "0.1.0",
			CParameterSet(I_TimeFrequency, I_Stimulations, S_ChannelLocalisation, S_DataScale, S_Caption, S_ColorGradient, P_None),
			"Displays the input matrices as a map of colored tiles, or bitmap, continuously.", ""))

	OVP_Declare_New(
		Topography2D("2D Topography", OID(0x0A0C12A7E695F3FB), OID(0x7C3A05B8C45386F8), "0.0.0", "0.0.0",
	...
	OVP_Declare_New(
		Topography3D("3D Topography", OID(0xA3F1CF20DEC477F3), OID(0xC709EA84B577D910), "0.0.0", "0.0.0",
			CParameterSet(I_Signal, S_ChannelLocalisation, S_DataScale, S_Caption, S_ColorGradient, F_FixedChannelOrder, P_None),
			"The input is mapped to a 3D model of the scalp, using a color gradient and interpolation.", ""))
	OVP_Declare_New(
		Cubes("3D Cubes", OID(0x6307469DF938EF27), OID(0xE028305D4ACF9D1A), "0.0.0", "0.0.0",
			CParameterSet(I_Signal, S_ChannelLocalisation, S_DataScale, S_Caption, S_ColorGradient, F_FixedChannelOrder, P_None),
	...
	OVP_Declare_New(
		SLoreta("3D Tomographic Visualization", OID(0xD5F62E7685E0D97C), OID(0xFD0826035C47E922), "0.0.0", "0.0.0",
