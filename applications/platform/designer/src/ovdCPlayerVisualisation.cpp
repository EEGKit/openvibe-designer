...
{
	CPlayerVisualisation* l_pVisualisation = reinterpret_cast<CPlayerVisualisation*>(data);
	CInterfacedScenario& l_rInterfacedScenario = l_pVisualisation->getInterfacedScenario();
	...
	else if(pVisualisationWidget->getType() == EVisualisationWidget_VisualisationWindow)
	{
		...
		if(l_bCreateWindow == true)
		{
			...
			gtk_window_set_default_size(
				...
				pVisualisationWidget->getWidth(),
				pVisualisationWidget->getHeight());
			//set its title
...
void CPlayerVisualisation::endLoadTreeWidget(OpenViBEVisualizationToolkit::IVisualisationWidget* pVisualisationWidget)
{
...
void CPlayerVisualisation::resizeCB(::GtkContainer* container)
{
	...
	else if(GTK_IS_PANED(container))
	{
		...
		int l_i32HandlePos = l_pVisualisationWidget->getDividerPosition();
		int l_i32MaxHandlePos = l_pVisualisationWidget->getMaxDividerPosition();
