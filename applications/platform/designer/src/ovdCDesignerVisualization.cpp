...
void CDesignerVisualization::init(const std::string& guiFile)
{
	...
	if (m_tree.getNextVisualizationWidgetIdentifier(windowID, VisualizationWidget_VisualizationWindow))
	{
...

{
	...
	if (m_tree.getNextVisualizationWidgetIdentifier(l_oVisualizationWindowID, VisualizationWidget_VisualizationWindow))
	{
		IVisualizationWidget* l_pVisualizationWindow = m_tree.getVisualizationWidget(l_oVisualizationWindowID);
		m_previewWindowW                             = l_pVisualizationWindow->getWidth();
		m_previewWindowH                             = l_pVisualizationWindow->getHeight();
	}
	const size_t treeViewWidth = gtk_paned_get_position(GTK_PANED(m_pane));
	gtk_widget_set_size_request(GTK_WIDGET(m_dialog), gint(treeViewWidth + m_previewWindowW), gint(m_previewWindowH));
...
void CDesignerVisualization::onVisualizationBoxAdded(const IBox* box)
{
	...
	m_tree.addVisualizationWidget(widgetID, box->getName(), VisualizationWidget_VisualizationBox, OV_UndefinedIdentifier, 0, box->getIdentifier(), 0,
...
GtkWidget* CDesignerVisualization::loadTreeWidget(IVisualizationWidget* widget)
{
	...
	if (widget->getType() == VisualizationWidget_VisualizationPanel)
	{
		...
		if (window != nullptr)
		{
			...
			else //otherwise retrieve it from first panel
			{
				...
				m_tree.getPointerValueFromTreeIter(&firstPanelIter, notebookWidget, EVisualizationTreeColumn_PointerWidget);
			 ...
			 widget->getType() == VisualizationWidget_Undefined || widget->getType() == VisualizationWidget_VisualizationBox)
	{
		...
		if (widget->getType() == VisualizationWidget_VerticalSplit || widget->getType() == VisualizationWidget_HorizontalSplit)
		{
			...
			GtkWidget* paned = (widget->getType() == VisualizationWidget_HorizontalSplit) ? gtk_hpaned_new() : gtk_vpaned_new();
		...
		if (parentWidget != nullptr) //visualization boxes may be unparented
		{
			...
			if (parentWidget->getType() == VisualizationWidget_VisualizationPanel)
			{
				...
				m_tree.getPointerValueFromTreeIter(&parentIter, notebook, EVisualizationTreeColumn_PointerWidget);
				char* panelName = nullptr;
				m_tree.getStringValueFromTreeIter(&parentIter, panelName, EVisualizationTreeColumn_StringName);
					 ...
					 VisualizationWidget_HorizontalSplit)
			{
				...
				m_tree.getPointerValueFromTreeIter(&parentIter, parentTreeWidget, EVisualizationTreeColumn_PointerWidget);
...
void CDesignerVisualization::endLoadTreeWidget(IVisualizationWidget* widget)
{
	...
	GtkTreeIter l_oIter;
	m_tree.findChildNodeFromRoot(&l_oIter, widget->getIdentifier());
	void* l_pTreeWidget = nullptr;
	m_tree.getPointerValueFromTreeIter(&l_oIter, l_pTreeWidget, EVisualizationTreeColumn_PointerWidget);
	...
	else if (widget->getType() == VisualizationWidget_Undefined || widget->getType() == VisualizationWidget_VisualizationBox)
	{
		if (GTK_IS_BUTTON(l_widget) != FALSE)
		{
			...
			m_tree.getStringValueFromTreeIter(&l_oIter, iconString, EVisualizationTreeColumn_StringStockIcon);
			//retrieve hbox
			...
			if (iconWidthRequest == 0) { gtk_widget_set_size_request(icon, 0, 0); }
			gtk_box_pack_start(GTK_BOX(l_pBox), icon, iconExpand, iconFill, 0);
...
const char* CDesignerVisualization::getTreeWidgetIcon(const EVisualizationTreeNode type)
{
	switch (type)
	{
		case EVisualizationTreeNode_Unaffected: return GTK_STOCK_DIALOG_QUESTION;
		case EVisualizationTreeNode_Undefined: return GTK_STOCK_CANCEL;
		case EVisualizationTreeNode_VisualizationWindow: return GTK_STOCK_DND_MULTIPLE;
		case EVisualizationTreeNode_VisualizationPanel: return GTK_STOCK_DND;
		case EVisualizationTreeNode_VisualizationBox: return GTK_STOCK_EXECUTE; //default (actual icon name may be retrieved from box descriptor)
		case EVisualizationTreeNode_HorizontalSplit:
		case EVisualizationTreeNode_VerticalSplit: return GTK_STOCK_ADD;
		default: return "";
	}
}
...
void CDesignerVisualization::resizeCB(IVisualizationWidget* widget)
{
	if (widget == nullptr)
	{
		...
		GtkWidget* l_pNotebook = gtk_paned_get_child2(GTK_PANED(m_pane));
		if (l_pNotebook != nullptr)
		{
			...
			{
				l_pNotebook = gtk_paned_get_child2(GTK_PANED(m_pane));
				if (l_pNotebook != nullptr)
				{
			...
			while (m_tree.getNextVisualizationWidgetIdentifier(l_oVisualizationWindowID, VisualizationWidget_VisualizationWindow))
			{
				IVisualizationWidget* l_pVisualizationWindow = m_tree.getVisualizationWidget(l_oVisualizationWindowID);
		...
		if (!m_tree.findChildNodeFromRoot(&l_oWindowIter, m_activeVisualizationWindowName, EVisualizationTreeNode_VisualizationWindow)) { return; }
		GtkTreeIter l_oPanelIter = l_oWindowIter;
		if (!m_tree.findChildNodeFromParent(&l_oPanelIter, m_activeVisualizationPanelName, EVisualizationTreeNode_VisualizationPanel)) { return; }
		CIdentifier l_oVisualizationPanelID;
		if (!m_tree.getIdentifierFromTreeIter(&l_oPanelIter, l_oVisualizationPanelID, EVisualizationTreeColumn_StringIdentifier)) { return; }
		IVisualizationWidget* l_pVisualizationPanel = m_tree.getVisualizationWidget(l_oVisualizationPanelID);
		...
		if (l_pVisualizationPanel != nullptr)
		{
			...
			IVisualizationWidget* l_pChildVisualizationWidget = m_tree.getVisualizationWidget(l_oChildID);
	...
	{
		GtkTreeIter l_oIter;
		if (m_tree.findChildNodeFromRoot(&l_oIter, widget->getIdentifier()) == TRUE)
		{
			//retrieve paned widget
			void* treeWidget = nullptr;
			m_tree.getPointerValueFromTreeIter(&l_oIter, treeWidget, EVisualizationTreeColumn_PointerWidget);
			...
			//go down child 1
			CIdentifier l_oChildID;
			widget->getChildIdentifier(0, l_oChildID);
			IVisualizationWidget* l_pChildVisualizationWidget = m_tree.getVisualizationWidget(l_oChildID);
			if (l_pChildVisualizationWidget != nullptr) { resizeCB(l_pChildVisualizationWidget); }

			//go down child 2
			widget->getChildIdentifier(1, l_oChildID);
			l_pChildVisualizationWidget = m_tree.getVisualizationWidget(l_oChildID);
			if (l_pChildVisualizationWidget != nullptr) { resizeCB(l_pChildVisualizationWidget); }
		}
	}
}
...
{
	//ensures correct behavior when _active[Window/Panel] point to m_oActiveVisualization[Window/Panel]Name.m_impl
	...
	if (m_tree.findChildNodeFromRoot(&windowIter, window, EVisualizationTreeNode_VisualizationWindow)) { m_activeVisualizationWindowName = window; }
	else
	{
		//pick first window if previously active window doesn't exist anymore
		CIdentifier id = OV_UndefinedIdentifier;

		if (m_tree.getNextVisualizationWidgetIdentifier(id, VisualizationWidget_VisualizationWindow))
		{
	...
	if (m_tree.findChildNodeFromParent(&panelIter, panel, EVisualizationTreeNode_VisualizationPanel)) { m_activeVisualizationPanelName = panel; }
	else //couldn't find panel : select first one
	{
		CIdentifier windowID;
		m_tree.getIdentifierFromTreeIter(&windowIter, windowID, EVisualizationTreeColumn_StringIdentifier);
		IVisualizationWidget* window = m_tree.getVisualizationWidget(windowID);
		CIdentifier panelID;
		if (window->getChildIdentifier(0, panelID))
		{
			...
			char* str = nullptr;
			m_tree.getStringValueFromTreeIter(&panelIter, str, EVisualizationTreeColumn_StringName);
	...
	void* notebook = nullptr;
	m_tree.getPointerValueFromTreeIter(&panelIter, notebook, EVisualizationTreeColumn_PointerWidget);
	GtkWidget* l_pCurrentNotebook = gtk_paned_get_child2(GTK_PANED(m_pane));
	if (l_pCurrentNotebook != GTK_WIDGET(notebook))
	{
		if (l_pCurrentNotebook != nullptr)
		{
			...
			gtk_container_remove(GTK_CONTAINER(m_pane), l_pCurrentNotebook);
		}
...
bool CDesignerVisualization::newVisualizationWindow(const char* label)
{
	...
	while (m_tree.getNextVisualizationWidgetIdentifier(windowID, VisualizationWidget_VisualizationWindow))
	{
	...
	m_tree.addVisualizationWidget(childID, childName, VisualizationWidget_VisualizationPanel, windowID, 0, OV_UndefinedIdentifier, 1, OV_UndefinedIdentifier);
...
bool CDesignerVisualization::renameVisualizationWindow(const char* label)
{
	...
	GtkTreeIter iter;
	if (!m_tree.findChildNodeFromRoot(&iter, m_activeVisualizationWindowName, EVisualizationTreeNode_VisualizationWindow))
	{
	...
	CIdentifier windowID;
	m_tree.getIdentifierFromTreeIter(&iter, windowID, EVisualizationTreeColumn_StringIdentifier);
	IVisualizationWidget* window = m_tree.getVisualizationWidget(windowID);
	if (window == nullptr)
	{
		displayErrorDialog("Window renaming failed !", "Couldn't retrieve window.");
	...
	//ensure name is unique
	CIdentifier id = OV_UndefinedIdentifier;
	while (m_tree.getNextVisualizationWidgetIdentifier(id, VisualizationWidget_VisualizationWindow))
	{
...
bool CDesignerVisualization::newVisualizationPanel(const char* label)
{
	...
	m_tree.addVisualizationWidget(childID, newName, VisualizationWidget_VisualizationPanel, windowID, window->getNbChildren(), OV_UndefinedIdentifier, 1,
								  OV_UndefinedIdentifier);
...
bool CDesignerVisualization::renameVisualizationPanel(const char* label)
{
	//retrieve visualization window
	GtkTreeIter iter;
	if (!m_tree.findChildNodeFromRoot(&iter, m_activeVisualizationWindowName.toASCIIString(), EVisualizationTreeNode_VisualizationWindow))
	{
	...
	CIdentifier panelID;
	m_tree.getIdentifierFromTreeIter(&iter, panelID, EVisualizationTreeColumn_StringIdentifier);
...
bool CDesignerVisualization::removeVisualizationPanel()
{
	//retrieve visualization window
	GtkTreeIter iter;
	m_tree.findChildNodeFromRoot(&iter, m_activeVisualizationWindowName.toASCIIString(), EVisualizationTreeNode_VisualizationWindow);

	//retrieve visualization panel
	m_tree.findChildNodeFromParent(&iter, m_activeVisualizationPanelName.toASCIIString(), EVisualizationTreeNode_VisualizationPanel);
	CIdentifier panelID;
	m_tree.getIdentifierFromTreeIter(&iter, panelID, EVisualizationTreeColumn_StringIdentifier);
...
bool CDesignerVisualization::removeVisualizationWidget()
{
	...
	CIdentifier id;
	m_tree.getIdentifierFromTreeIter(&iter, id, EVisualizationTreeColumn_StringIdentifier);
...
bool CDesignerVisualization::removeVisualizationWidget(const CIdentifier& identifier)
{
	...
	if (widget->getType() != VisualizationWidget_VisualizationBox) { m_tree.destroyHierarchy(identifier, false); }
	...
	if (GTK_IS_TREE_VIEW(widget))
	{
		if (event->button == 3) //right button
		{
			if (event->type != GDK_BUTTON_PRESS)
			{
				...
				if (!m_tree.getTreeSelection(m_treeView, &l_oIter)) { return; }

				const unsigned long type = m_tree.getULongValueFromTreeIter(&l_oIter, EVisualizationTreeColumn_ULongNodeType);
				...
				else if (type == EVisualizationTreeNode_VisualizationBox)
				{
					...
					if (m_tree.findParentNode(&l_oIter, EVisualizationTreeNode_VisualizationPanel))
					{
				...
				else if (type == EVisualizationTreeNode_Undefined)
				{
					...
					CIdentifier id;
					m_tree.getIdentifierFromTreeIter(&l_oIter, id, EVisualizationTreeColumn_StringIdentifier);
					IVisualizationWidget* l_pVisualizationWidget = m_tree.getVisualizationWidget(id);
					if (l_pVisualizationWidget != nullptr)
					{
						IVisualizationWidget* l_pParentVisualizationWidget = m_tree.getVisualizationWidget(
...
void CDesignerVisualization::cursorChanged(GtkTreeView* treeView)
{
	...
	if (!m_tree.getTreeSelection(treeView, &selectionIter)) { return; }

	//save active item
	if (m_tree.getULongValueFromTreeIter(&selectionIter, EVisualizationTreeColumn_ULongNodeType) == EVisualizationTreeNode_VisualizationBox)
	{
		m_tree.getIdentifierFromTreeIter(&selectionIter, m_activeVisualizationBoxID, EVisualizationTreeColumn_StringIdentifier);
	}

	GtkTreeIter panelIter = selectionIter;

	//if selection lies in a visualization panel subtree, display this subtree
	if (m_tree.findParentNode(&panelIter, EVisualizationTreeNode_VisualizationPanel))
	{
		//get visualization panel name
		char* panelName = nullptr;
		m_tree.getStringValueFromTreeIter(&panelIter, panelName, EVisualizationTreeColumn_StringName);

		//retrieve visualization window that contains selection
		GtkTreeIter windowIter = panelIter;
		if (m_tree.findParentNode(&windowIter, EVisualizationTreeNode_VisualizationWindow))
		{
			//get its name
			char* windowName = nullptr;
			m_tree.getStringValueFromTreeIter(&windowIter, windowName, EVisualizationTreeColumn_StringName);
	...
	{
		GtkTreeIter windowIter = selectionIter;
		...
		if (m_tree.findParentNode(&windowIter, EVisualizationTreeNode_VisualizationWindow))
		{
			//retrieve visualization window
			CIdentifier windowID;
			m_tree.getIdentifierFromTreeIter(&windowIter, windowID,
...
void CDesignerVisualization::dragDataReceivedInWidget(GtkWidget* dstWidget, GtkSelectionData* selection)
{
	...
	CIdentifier srcID;
	m_tree.getIdentifierFromTreeIter(&srcIter, srcID, EVisualizationTreeColumn_StringIdentifier);
	...
	if (srcVisualizationWidget->getParentIdentifier() == OV_UndefinedIdentifier ||
		m_tree.getULongValueFromTreeIter(&dstIter, EVisualizationTreeColumn_ULongNodeType) == EVisualizationTreeNode_VisualizationBox)
	{
	...
	else //dest widget is a dummy : unaffect src widget and simplify the tree before performing the drop operation
	{
		...
		CIdentifier dstID;
		m_tree.getIdentifierFromTreeIter(&dstIter, dstID, EVisualizationTreeColumn_StringIdentifier);
		...
		m_tree.getPointerValueFromTreeIter(&dstIter, newDstTreeWidget, EVisualizationTreeColumn_PointerWidget);
																...
																GtkSelectionData* selection, guint /*info*/, guint /*time*/, gpointer data)
{
	...
	EDragDataLocation location;
	if (strcmp(buf, "left") == 0) { location = EDragData_Left; }
	else if (strcmp(buf, "right") == 0) { location = EDragData_Right; }
	else if (strcmp(buf, "top") == 0) { location = EDragData_Top; }
	else { location = EDragData_Bottom; }

	static_cast<CDesignerVisualization*>(visualization)->dragDataReceivedInEventBox(dstWidget, selection, location);
}

void CDesignerVisualization::dragDataReceivedInEventBox(GtkWidget* dstWidget, GtkSelectionData* selection, const EDragDataLocation location)
{
	void* srcWidget = nullptr;
	sscanf(reinterpret_cast<const char*>(gtk_selection_data_get_text(selection)), "%p", &srcWidget);
	GtkTreeIter srcIter;
	...
	if (GTK_IS_TREE_VIEW(srcWidget))
	{
		...
		m_tree.getPointerValueFromTreeIter(&srcIter, srcWidget, EVisualizationTreeColumn_PointerWidget);
	...
	if (m_tree.getULongValueFromTreeIter(&srcIter, EVisualizationTreeColumn_ULongNodeType) != EVisualizationTreeNode_VisualizationBox) { return; }
	...
	if (m_tree.findParentNode(&unaffectedIter, EVisualizationTreeNode_Unaffected)) { m_tree.dragDataReceivedOutsideWidgetCB(srcID, dstWidget, location); }
	else
	{
		//save dest widget identifier
		GtkTreeIter dstIter;
