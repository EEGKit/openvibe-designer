...
boolean CCommentEditorDialog::run(void)
{
	...
	m_pInterface=gtk_builder_new(); // glade_xml_new(m_sGUIFilename.toASCIIString(), "comment", NULL);
	gtk_builder_add_from_file(m_pInterface, m_sGUIFilename.toASCIIString(), NULL);
	...
	::g_signal_connect(m_pInfoDialog, "close", G_CALLBACK(::gtk_widget_hide), NULL);
	::g_signal_connect(m_pInfoDialog, "delete-event", G_CALLBACK(::gtk_widget_hide), NULL);
	
	//::g_signal_connect(GTK_WIDGET(gtk_builder_get_object(m_pInterface, "messagedialog_howto_comment_button_close")), "clicked", G_CALLBACK(::gtk_widget_hide), NULL);

	g_object_unref(m_pInterface);

	m_pDescriptionBuffer=gtk_text_buffer_new(NULL);
...
void CCommentEditorDialog::infoCB(void)
{
