...
struct SBoxProto final : Kernel::IBoxProto
{
	...
	void swap_byte(uint64_t& v, const uint64_t s) const
	{
		uint8_t V[sizeof(v)];
		uint8_t S[sizeof(s)];
...
static void drag_data_get_cb(GtkWidget* widget, GdkDragContext* dc, GtkSelectionData* selectionData, const guint info, const guint time, gpointer data)
{
...
static void queue_refilter(CApplication* app)
{
	...
	app->m_FilterTimeout = gint(g_timeout_add(300, GSourceFunc(do_refilter), app));
}
