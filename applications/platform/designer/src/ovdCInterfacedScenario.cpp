...
*/
	...
	static void context_menu_cb(::GtkMenuItem* pMenuItem, CInterfacedScenario::BoxContextMenuCB* pContextMenuCB)
	{
		...
		switch(pContextMenuCB->ui32Command)
		{
			...
			case ContextMenu_BoxAddMessageInput:       pContextMenuCB->pInterfacedScenario->contextMenuBoxAddMessageInputCB(*pContextMenuCB->pBox); break;
			case ContextMenu_BoxRemoveMessageInput:    pContextMenuCB->pInterfacedScenario->contextMenuBoxRemoveMessageInputCB(*pContextMenuCB->pBox, pContextMenuCB->ui32Index); break;
			case ContextMenu_BoxAddMessageOutput:      pContextMenuCB->pInterfacedScenario->contextMenuBoxAddMessageOutputCB(*pContextMenuCB->pBox); break;
			case ContextMenu_BoxRemoveMessageOutput:   pContextMenuCB->pInterfacedScenario->contextMenuBoxRemoveMessageOutputCB(*pContextMenuCB->pBox, pContextMenuCB->ui32Index); break;

			case ContextMenu_BoxEditMessageInput:      pContextMenuCB->pInterfacedScenario->contextMenuBoxEditMessageInputCB(*pContextMenuCB->pBox, pContextMenuCB->ui32Index); break;
			case ContextMenu_BoxEditMessageOutput:     pContextMenuCB->pInterfacedScenario->contextMenuBoxEditMessageOutputCB(*pContextMenuCB->pBox,  pContextMenuCB->ui32Index); break;
...
//	gtk_builder_add_from_file(m_pSettingsGUIBuilder, m_sGUISettingsFilename.c_str(), NULL);
	...
	*/
	...
	if(m_rKernelContext.getConfigurationManager().expandAsBoolean("${Kernel_WithGUI}", true))
	{
...
void CInterfacedScenario::redrawScenarioInputSettings()
{
	...
	boolean (IScenario::*l_pfGetLinkName)(uint32, CString&) const = &IScenario::getInputName;
	boolean (IScenario::*l_pfGetLinkType)(uint32, CIdentifier&) const = &IScenario::getInputType;
...
void CInterfacedScenario::redrawScenarioOutputSettings()
{
	...
	boolean (IScenario::*l_pfGetLinkName)(uint32, CString&) const = &IScenario::getOutputName;
	boolean (IScenario::*l_pfGetLinkType)(uint32, CIdentifier&) const = &IScenario::getOutputType;
...
void CInterfacedScenario::redrawScenarioLinkSettings(
		...
		boolean (IScenario::*pfGetLinkName)(uint32, CString&) const,
		boolean (IScenario::*pfGetLinkType)(uint32, CIdentifier&) const
		)
{
	GList* l_pSettingWidgets;
	GList* l_pSettingIterator;
...
void CInterfacedScenario::scenarioDrawingAreaExposeCB(::GdkEventExpose* pEvent)
{
	if(m_ui32CurrentMode==Mode_None)
	{
		...
		if(l_iNewScenarioSizeX>=0 && l_iNewScenarioSizeY>=0)
		{
			...
			if( l_iMaxX + m_i32ViewOffsetX>-l_iMarginX + std::max(l_iViewportX, l_iNewScenarioSizeX + 2 * l_iMarginX) ) { m_i32ViewOffsetX = -l_iMaxX - l_iMarginX + std::max(l_iViewportX, l_iNewScenarioSizeX + 2 * l_iMarginX); }
			if(l_iMinX+m_i32ViewOffsetX< l_iMarginX)                                                     { m_i32ViewOffsetX=-l_iMinX+l_iMarginX; }
			if( l_iMaxY + m_i32ViewOffsetY>-l_iMarginY + std::max(l_iViewportY, l_iNewScenarioSizeY + 2 * l_iMarginY) ) { m_i32ViewOffsetY = -l_iMaxY - l_iMarginY + std::max(l_iViewportY, l_iNewScenarioSizeY + 2 * l_iMarginY); }
...
void CInterfacedScenario::scenarioDrawingAreaDragDataReceivedCB(::GdkDragContext* pDragContext, gint iX, gint iY, ::GtkSelectionData* pSelectionData, guint uiInfo, guint uiT)
{
	...
	if(pDragContext->protocol == GDK_DRAG_PROTO_WIN32_DROPFILES)
	{
		...
		for(uint32 i = 0; i < l_vFilesToOpen.size(); i++)
		{
			m_rApplication.openScenario(l_vFilesToOpen[i].c_str());
		}
				...
				case GDK_BUTTON_PRESS:
					{
						...
						if(m_oCurrentObject.m_oIdentifier!=OV_UndefinedIdentifier && m_rScenario.isBox(m_oCurrentObject.m_oIdentifier))
						{
							...
							if(l_pBox)
							{
								...
								boolean l_bCanConnectScenarioInput=(l_pBox->getInputCount()>0 && m_rScenario.getInputCount()>0);
								if(l_bFlagCanAddInput || l_bFlagCanModifyInput || l_bCanConnectScenarioInput)
								{
									...
									for(i=0; i<l_pBox->getInputCount(); i++)
									{
										...
										{
											g_object_unref(l_pMenuInputMenuAction);
								...
								boolean l_bCanConnectScenarioOutput=(l_pBox->getOutputCount()>0 && m_rScenario.getOutputCount()>0);
								...
								if (m_rApplication.m_pMetaboxLoader != NULL)
								{
									if (l_pBox->getAlgorithmClassIdentifier() == OVP_ClassId_BoxAlgorithm_Metabox)
									{
										...
										l_bIsFileBinary = (::strlen(l_sMetaboxScenarioPath) > 4  && (_strcmpi(l_sMetaboxScenarioPath+strlen(l_sMetaboxScenarioPath)-4, ".mbs") == 0 ||
																									 _strcmpi(l_sMetaboxScenarioPath+strlen(l_sMetaboxScenarioPath)-4, ".mbb") == 0));
										if(!l_bIsFileBinary || m_rApplication.m_bCanImportBinary)
										{
		...
		for(it=m_vCurrentObject.begin(); it!=m_vCurrentObject.end(); it++)
		{
			if(it->second)
			{
				if(m_rScenario.isBox(it->first))
				{
					...
					l_vSelectedBoxAlgorithm[it->first]=true;
				}
			}
		}
...
void CInterfacedScenario::contextMenuBoxToggleEnableAllCB(void)
{
	...
	for(it_current=m_vCurrentObject.begin(); it_current!=m_vCurrentObject.end(); it_current++)
	{
		if(it_current->second)
		{
			if(m_rScenario.isBox(it_current->first))
			{
				TAttributeHandler l_oAttributeHandler(*m_rScenario.getBoxDetails(it_current->first));
...
void CInterfacedScenario::createPlayerVisualisation(IVisualisationTree* pVisualisationTree)
{
	//hide window manager
	if(m_pDesignerVisualisation)
	{
	...
	if(m_pPlayerVisualisation == NULL)
	{
		if (pVisualisationTree)
		{
			m_pPlayerVisualisation = new CPlayerVisualisation(m_rKernelContext, *pVisualisationTree, *this);
		}
		else
		{
