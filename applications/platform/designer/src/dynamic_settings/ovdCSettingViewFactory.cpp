...
CSettingViewFactory::CSettingViewFactory(const CString& rBuilderName, const Kernel::IKernelContext& rKernelContext)
	: m_builderName(rBuilderName), m_rKernelContext(rKernelContext) {}
...
CAbstractSettingView* CSettingViewFactory::getSettingView(Kernel::IBox& rBox, const uint32_t index)
{
	...
	if (l_oSettingType == OV_TypeId_Integer) { return new CIntegerSettingView(rBox, index, m_builderName, m_rKernelContext); }
	if (l_oSettingType == OV_TypeId_Float) { return new CFloatSettingView(rBox, index, m_builderName, m_rKernelContext); }
	if (l_oSettingType == OV_TypeId_String) { return new CStringSettingView(rBox, index, m_builderName); }
	if (l_oSettingType == OV_TypeId_Filename) { return new CFilenameSettingView(rBox, index, m_builderName, m_rKernelContext); }
	if (l_oSettingType == OV_TypeId_Script) { return new CScriptSettingView(rBox, index, m_builderName, m_rKernelContext); }
	if (l_oSettingType == OV_TypeId_Color) { return new CColorSettingView(rBox, index, m_builderName, m_rKernelContext); }
	if (l_oSettingType == OV_TypeId_ColorGradient) { return new CColorGradientSettingView(rBox, index, m_builderName, m_rKernelContext); }
	if (m_rKernelContext.getTypeManager().isEnumeration(l_oSettingType))
	{
		return new CEnumerationSettingView(rBox, index, m_builderName, m_rKernelContext, l_oSettingType);
	}
	if (m_rKernelContext.getTypeManager().isBitMask(l_oSettingType))
	{
		return new CBitMaskSettingView(rBox, index, m_builderName, m_rKernelContext, l_oSettingType);
	}
